...
- 支持导入自定义解析源，当网易云无法解析播放地址时自动尝试备用音源。
...
应用支持在「设置 - 音乐」中导入自定义解析源，可以选择本地 JavaScript 文件，也可以直接粘贴在线解析源链接。

自定义解析源不会替代正常的网易云播放链路。对于无版权、已下架等歌曲，列表会保留播放入口，并在网易云无法播放时尝试使用自定义解析源解析。

如果需要现成的在线解析源链接，可以参考 [pdone/lx-music-source](https://github.com/pdone/lx-music-source) 的说明。
