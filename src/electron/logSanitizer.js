...
function sanitizeLogValue(value, seen = new WeakSet()) {
    ...
    if (Buffer.isBuffer(value)) {
        return value.length >= BASE64_LOG_MIN_LENGTH ? `[Buffer ${value.length} bytes]` : value
    }
...
installLogSanitizer.sanitizeString = sanitizeString
installLogSanitizer.sanitizeLogValue = sanitizeLogValue
installLogSanitizer.sanitizeLogArgs = sanitizeLogArgs

module.exports = installLogSanitizer
