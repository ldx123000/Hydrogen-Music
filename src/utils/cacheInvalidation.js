...
const PLAYLIST_CACHE_INVALIDATION_TTL_MS = 5000;
...
function appendNeedTimestampUrls(needTimestamp, urls) {
  const nextNeedTimestamp = removeNeedTimestampUrls(
    dedupeNeedTimestampUrls(needTimestamp),
    urls,
  );
...
export function scheduleCacheInvalidationWindow(
  key,
  urls,
  ttlMs = DEFAULT_CACHE_INVALIDATION_TTL_MS,
) {
  ...
  libraryStore.needTimestamp = appendNeedTimestampUrls(
    libraryStore.needTimestamp,
    normalizedUrls,
  );
  ...
  const timerId = setTimeout(() => {
    ...
    currentLibraryStore.needTimestamp = removeNeedTimestampUrls(
      currentLibraryStore.needTimestamp,
      normalizedUrls,
    );
...
export function schedulePlaylistCacheInvalidation(
  ttlMs = PLAYLIST_CACHE_INVALIDATION_TTL_MS,
) {
  scheduleCacheInvalidationWindow(
    CACHE_INVALIDATION_KEYS.playlist,
    CACHE_INVALIDATION_URLS.playlist,
    ttlMs,
  );
}

export function scheduleAlbumSublistCacheInvalidation(
  ttlMs = DEFAULT_CACHE_INVALIDATION_TTL_MS,
) {
  scheduleCacheInvalidationWindow(
    CACHE_INVALIDATION_KEYS.albumSublist,
    CACHE_INVALIDATION_URLS.albumSublist,
    ttlMs,
  );
}

export function scheduleArtistSublistCacheInvalidation(
  ttlMs = DEFAULT_CACHE_INVALIDATION_TTL_MS,
) {
  scheduleCacheInvalidationWindow(
    CACHE_INVALIDATION_KEYS.artistSublist,
    CACHE_INVALIDATION_URLS.artistSublist,
    ttlMs,
  );
}
