...
async function fetchAudioArrayBuffer(url) {
    ...
    const timeout = setTimeout(() => controller.abort(), DEFAULT_AUDIO_FETCH_TIMEOUT_MS)
...
class WebAudioBufferPlayer {
    ...
    _startSource(offset) {
        ...
        this._startedAt = this._context.currentTime - this._offset
        this._playing = true
...
export async function createDecodedAudioPlayer(url) {
